...
| content | 是 | markdown_v2内容，最长不超过4096个字节，必须是utf8编码。markdown_v2不支持字体颜色、@群成员的语法|
...
| 一级标题     | 最多2行（13×2个字）                                                      |
| 标题辅助信息 | 最多2行（15×2个字）                                                      |
| 关键数据     | 最多1行（数字10个字 数字含义15个字）（建议不与引用文本共用）             |
| 引用文本     | 引用文本标题最多1行 引用文本最多3行 引用文本标题可不要（建议不与关键数据共用） |
| 二级普通文本 | 最多8行（14×8个字）                                                      |
| 二级标题+文本 | 二级标题最多5个字 二级信息内容最多2行（最多13×2个字） 二级标题+二级信息内容的组合最多6个 |
| 跳转指引     | 最多3个（跳转指引文案最多13个字）                                        |
...
|----------------------|--------------------------------------------------------------------------|
| 来源                 | 最多1行（13个字）                                                        |
| 图文混排（形式1）    | 一级信息标题最多2行（13×2个字） 标题辅助信息最多2行（15×2个字） 图片最多1个（2.25>图片宽高比>1.3） （建议不与形式2共用） |
| 图文混排（形式2）    | 一级信息标题最多1行（9个字） 标题辅助信息最多1行（11个字） （图片最大44x44） |
| 二级标题             | 最多2行（13×2个字）                                                      |
| 二级普通文本         | 最多8行（14×8个字）                                                      |
| 二级标题+文本        | 二级标题最多5个字 二级信息内容最多2行（最多13×2个字） 二级标题+二级信息内容的组合最多6个 |
| 跳转指引             | 最多3个（跳转指引文案最多13个字）                                        |
